        ...
        $query->where(function($where) use ($searchClientName)
{
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                if( $filter == "week" ) 
                {
                    $query->where("duedate", "<=", Carbon\Carbon::now()->addDays(7)->toDateString())->where("completed", "=", 0);
                }
                else
                {
                    if( $filter == "closed" ) 
    ...
    foreach( $query->get() as $data ) 
    {
        ...
        $editprojecthtml = ($show_project ? "<a href=\"" . str_replace("m=overview", "m=view", $modulelink) . "&projectid=" . $projectid . "\"><img src=\"images/edit.gif\" border=\"0\" /></a>" : "");
        $deleteprojecthtml = (project_management_checkperm("Delete Projects") ? "<a href=\"#\" onclick=\"doDelete('" . $projectid . "');return false\"><img src=\"images/delete.gif\" border=\"0\" /></a>" : "");
        $tabledata[] = array( "<div align=\"left\">" . $title . "</div>", $adminuser, $status, $created, $duedate, "<div class=\"project-progress\" data-percentage=\"" . $progressdata["percent"] . "\"></div>", $daysleft, $lastmodified, $editprojecthtml, $deleteprojecthtml );
    }
    $jquerycode .= "jQuery(function() {\n    jQuery('.project-progress').progressbar({\n        create: function(event) {\n            jQuery(event.target).progressbar('value', jQuery(event.target).data('percentage'));\n        }\n    });\n});";

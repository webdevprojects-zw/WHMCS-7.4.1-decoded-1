...
if( !$_REQUEST["failed"] ) 
{
    ...
    logTransaction($GATEWAY["paymentmethod"], $data, "Authenticate Response");
    ...
    logTransaction($GATEWAY["paymentmethod"], $query_str, "Payment Request");
    $data = curlCall($url, $query_str);
    $xmlData = XMLtoArray($data);
    $xmlData = $xmlData["CCTXNRESPONSEV1"];
    if( $xmlData["CODE"] == "0" ) 
    {
        ...
        logTransaction($GATEWAY["paymentmethod"], $data, "Approved");
        sendMessage("Credit Card Payment Confirmation", $invoiceID);
        $callbackSuccess = true;
    }
    else
    {
        logTransaction($GATEWAY["paymentmethod"], $data, "Declined");

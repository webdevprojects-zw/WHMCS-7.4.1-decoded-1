...
if( $module && !$aid ) 
{
    ...
    {
        logActivity("Required Product Module '" . $serverModule->getServiceModule() . "' Missing - Service ID: " . $id);
    }
...
if( $frm->issubmitted() ) 
{
    ...
    if( $aid ) 
    {
        ...
        else
        {
            ...
            if( $addonid ) 
            {
                ...
                {
                    $billingcycle = $productAddon->billingCycle;
                    $currency = getCurrency($userid);
                    $pricing = new WHMCS\Pricing();
                    $pricing->loadPricing("addon", $addonid, $currency["id"]);
                    switch( $billingcycle ) 
                    {
                        case "recurring":
    ...
    foreach( $updatefields as $fieldname ) 
    {
        ...
        {
            $updatearr["completed_date"] = Carbon\Carbon::today()->toDateString();
        }
...
{
    check_token("WHMCS.admin.default");
    $result = ServerTerminateAccount($id, (int) $aid);
    WHMCS\Cookie::set("ModCmdResult", $result);
    $extra = "";
    if( (int) $aid ) 
    {
...
$predefaddons = WHMCS\Product\Addon::pluck("name", "id")->all();
$addonServices = array(  );
$result = select_query("tblhostingaddons", "id,hostingid,addonid,name,status", array( "userid" => $userid ), "name", "ASC");
while( $data = mysql_fetch_array($result) ) 
{
    ...
    $addonServices[$data["hostingid"]]["a" . $data["id"]] = array( $color, "- " . (($data["name"] ? $data["name"] : $predefaddons[$data["addonid"]])) );
}
$allServices = array(  );
$servicesarr = array(  );
...
if( !$aid ) 
{
    echo "<div class=\"col-sm-5\">\n            " . $frm->button("<i class=\"fa fa-arrow-circle-up\"></i> " . $aInt->lang("services", "createupgorder"), "window.open('clientsupgrade.php?id=" . $id . "','','width=750,height=350,scrollbars=yes')") . "\n            " . $frm->button("<i class=\"fa fa-random\"></i> " . $aInt->lang("services", "moveservice"), "window.open('clientsmove.php?type=hosting&id=" . $id . "','movewindow','width=500,height=300,top=100,left=100,scrollbars=yes')") . "\n        </div>";
}
...
if( $aid ) 
{
    ...
    $tbl->add($aInt->lang("fields", "regdate"), $frm->date("regdate", $regdate, 12, false, "form-control date-picker"));
    ...
    $tbl->add($aInt->lang("fields", "nextduedate"), $frm->date("nextduedate", $nextduedate, 12, false, "form-control date-picker"));
    $tbl->add($aInt->lang("fields", "status"), $aInt->productStatusDropDown($status));
    $tbl->add(AdminLang::trans("fields.terminationDate"), $frm->date("termination_date", (strpos($terminationDate, "0000") === false ? $terminationDate : ""), 12, false, "form-control date-picker"));
...
else
{
    ...
    $tbl->add($aInt->lang("fields", "nextduedate"), (in_array($billingcycle, array( "One Time", "Free Account" )) ? AdminLang::trans("global.na") : $frm->hidden("oldnextduedate", $nextduedate) . $frm->date("nextduedate", $nextduedate, 12, false, "form-control date-picker")));
    $tbl->add($aInt->lang("fields", "dedicatedip"), $frm->text("dedicatedip", $dedicatedip, "25", false, "form-control input-200"));
    $tbl->add($aInt->lang("fields", "terminationDate"), $frm->date("termination_date", (strpos($terminationDate, "0000") === false ? $terminationDate : ""), 12, false, "form-control date-picker"));
    ...
    $tbl->add($aInt->lang("services", "overrideautosusp"), "<div class=\"pull-left\" style=\"padding-top:6px;\">" . $frm->checkbox("overideautosuspend", $aInt->lang("services", "nosuspenduntil"), $overideautosuspend) . " &nbsp;\n    </div>" . $frm->date("overidesuspenduntil", (strpos($overidesuspenduntil, "0000") === false ? $overidesuspenduntil : ""), 12, false, "form-control date-picker") . "</div>", 1);
...
$content .= $aInt->modal("ModuleTerminate", $aInt->lang("services", "confirmcommand"), $aInt->lang("services", "terminatesure"), array( array( "title" => $aInt->lang("global", "yes"), "onclick" => "runModuleCommand(\"terminate\")", "class" => "btn-primary" ), array( "title" => $aInt->lang("global", "no") ) ));

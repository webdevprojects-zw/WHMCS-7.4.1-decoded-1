    ...
    else
    {
            ...
            if( isset($_FILES["attachments"]) ) 
            {
                foreach( $_FILES["attachments"]["name"] as $num => $filename ) 
                {
                    try
                    {
                        ...
                        $attachments[] = array( "path" => $whmcs->getAttachmentsDir() . DIRECTORY_SEPARATOR . $filename, "filename" => $filename, "displayname" => $file->getCleanName() );
                    }
        ...
        if( $done ) 
        {
            ...
            foreach( $_SESSION["massmail"]["attachments"] as $parts ) 
            {
                try
                {
                    $file = new WHMCS\File($whmcs->getAttachmentsDir() . DIRECTORY_SEPARATOR . $parts["filename"]);
                    $file->delete();
                }

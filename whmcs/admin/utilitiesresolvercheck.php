...
if( $step == "" ) 
{
    ...
    echo "?step=2\">\n<select name=\"server\" onChange=\"submit()\" class=\"form-control select-inline\"><option value=\"\">Check All";
...
else
{
    if( $step == "2" ) 
    {
        ...
        echo "</p>\n\n";
        echo $aInt->modal("TerminateAccounts", $aInt->lang("utilitiesresolvercheck", "terminateonserver"), $aInt->lang("utilitiesresolvercheck", "delsureterminateonserver"), array( array( "title" => AdminLang::trans("global.yes"), "onclick" => "window.location=\"?step=terminate&\" + jQuery(\"#resolverfrm\").serialize();" ), array( "title" => AdminLang::trans("global.no") ) ));
    }
    else
    {
        if( $step == "terminate" ) 

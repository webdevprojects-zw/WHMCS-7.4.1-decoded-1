            ...
            while( $data = mysql_fetch_array($result) ) 
            {
                ...
                if( $showpending && $hostingstatus == "Pending" ) 
                {
                    ...
                    if( $servertype ) 
                    {
                        ...
                        $servers = Illuminate\Database\Capsule\Manager::table("tblservers")->where(function($query) use ($servertype)
{
    $query->where("type", "=", $servertype)->where("disabled", "=", 0);
}

)->orWhere("id", "=", $server)->orderBy("name")->get();
                        foreach( $servers as $listedServer ) 
                        {
                            ...
                            $serverNumberAccounts = WHMCS\Service\Service::where("server", "=", $listedServer->id)->whereIn("domainstatus", array( "Active", "Suspended" ))->count();
                            $selectedServer = ($listedServer->id == $server ? " selected" : "");
                            echo "<option value=\"" . $listedServer->id . "\"" . $selectedServer . ">\n    " . $serverName . " (" . $serverNumberAccounts . "/" . $listedServer->maxaccounts . ")\n</option>";
                        }
                        echo "</select> <label class=\"checkbox-inline\"><input type=\"checkbox\" name=\"vars[products][" . $hostingid . "][runcreate]\"";
            ...
            while( $data = mysql_fetch_array($result) ) 
            {
                ...
                echo "<tr><td align=\"center\"><a href=\"clientsservices.php?userid=" . $userid . "&id=" . $hostingid . "&aid=" . $aid . "\"><b>" . $aInt->lang("orders", "addon") . "</b></a></td><td>" . $name . "</td><td>" . $aInt->lang("billingcycles", str_replace(array( "-", "account", " " ), "", strtolower($billingcycle2))) . "</td><td>" . $addonamount . "</td><td>" . $aInt->lang("status", strtolower($addonstatus)) . "</td><td><b>" . $paymentstatus . "</td></tr>";
                if( $addonstatus == "Pending" && $predefinedaddons[$addonid]["welcomeemail"] ) 
                {
                    echo "<tr><td style=\"background-color:#EFF2F9;text-align:center;\" colspan=\"6\"><label class=\"checkbox-inline\"><input type=\"checkbox\" name=\"vars[addons][" . $aid . "][sendwelcome]\" checked> " . $aInt->lang("orders", "sendwelcome") . "</label></td></tr>";
                }

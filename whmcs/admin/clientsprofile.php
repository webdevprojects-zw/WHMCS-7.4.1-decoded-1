...
if( $whmcs->get_req_var("save") ) 
{
    ...
    else
    {
        ...
        $array = array( "uuid" => $uuid, "firstname" => $firstname, "lastname" => $lastname, "companyname" => $companyname, "email" => $email, "address1" => $address1, "address2" => $address2, "city" => $city, "state" => $state, "postcode" => $postcode, "country" => $country, "phonenumber" => $phonenumber, "currency" => $_POST["currency"], "notes" => $notes, "status" => $status, "taxexempt" => (bool) $taxexempt, "latefeeoveride" => (bool) $latefeeoveride, "overideduenotices" => (bool) $overideduenotices, "separateinvoices" => (bool) $separateinvoices, "disableautocc" => (bool) $disableautocc, "emailoptout" => (bool) $emailoptout, "overrideautoclose" => (bool) $overrideautoclose, "language" => $language, "billingcid" => $billingcid, "securityqid" => $securityqid, "securityqans" => encrypt($securityqans), "groupid" => $groupid, "allow_sso" => (bool) $whmcs->get_req_var("allowsinglesignon") );
        if( !$twofaenabled ) 
        {
            $array["authmodule"] = "";
            $array["authdata"] = "";
...
echo "</td><td class=\"fieldarea\"><label class=\"checkbox-inline\"><input type=\"checkbox\" name=\"emailoptout\"";
if( $emailoptout == "on" || $emailoptout == 1 ) 
{
    echo " checked";
}
...
echo "</td><td class=\"fieldarea\"><select name=\"groupid\" class=\"form-control select-inline\" tabindex=\"27\"><option value=\"0\">";
echo $aInt->lang("global", "none");
echo "</option>\n";

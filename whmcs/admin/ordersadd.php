...
else
{
    if( $action == "getdomainaddlfields" ) 
    {
        ...
        foreach( $additflds->getFieldsForOutput($domainCounter) as $fieldLabel => $inputHTML ) 
        {
            echo "<tr class=\"domain-addt-fields\"><td width=\"130\" class=\"fieldlabel\">" . $fieldLabel . "</td><td class=\"fieldarea\">" . $inputHTML . "</td></tr>" . PHP_EOL;
        }
        exit();
    }
    else
    {
        ...
        if( $whmcs->get_req_var("submitorder") ) 
        {
            ...
            else
            {
                ...
                foreach( $regaction as $k => $regact ) 
                {
                    ...
                    {
                        $domainArray = array( "type" => $regact, "domain" => $regdomain[$k], "regperiod" => $regperiod[$k], "dnsmanagement" => $dnsmanagement[$k], "emailforwarding" => $emailforwarding[$k], "idprotection" => $idprotection[$k], "eppcode" => $eppcode[$k], "fields" => $domainfield[$k] );
        ...
        if( checkPermission("Create/Edit Promotions", true) ) 
        {
            echo " <a href=\"#\" data-toggle=\"modal\" data-target=\"#modalCreatePromo\" class=\"btn btn-default btn-sm\"><img src=\"images/icons/add.png\" border=\"0\" align=\"absmiddle\" /> " . $aInt->lang("orders", "createpromo");
        }
        ...
        echo $aInt->lang("fields", "domain");
        echo "</td>\n        <td class=\"fieldarea\"><input type=\"text\" name=\"regdomain[]\" id=\"inputDomainRegDomain0\" class=\"form-control input-300 input-reg-domain\" data-manual-input=\"0\" onkeyup=\"\$(this).prop('data-manual-input', 1); handleDomainRegInput(this);\" /></td>\n    </tr>\n    <tr style=\"display:none;\">\n        <td class=\"fieldlabel\">";

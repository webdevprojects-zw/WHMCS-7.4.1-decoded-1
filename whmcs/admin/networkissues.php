...
if( $action == "" ) 
{
    ...
    if( mysql_num_rows($result) ) 
    {
        while( $open_row = mysql_fetch_assoc($result) ) 
        {
            ...
            $tabledata[] = array( "<a href=\"" . $_SERVER["PHP_SELF"] . "?action=manage&id=" . $open_row["id"] . "\">" . $open_row["title"] . "</a>", $open_row["type"], $open_row["priority"], $open_row["status"], fromMySQLDate($open_row["startdate"], true), $enddate, $actions, "<a href=\"networkissues.php#\" onClick=\"doDelete('" . $open_row["id"] . "');return false\"><img src=\"images/delete.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"Delete\"></a>" );
        }
    }
...
else
{
    if( $action == "manage" ) 
    {
        ...
        echo WHMCS\View\Asset::jsInclude("jquery-ui-timepicker-addon.js") . WHMCS\View\Asset::cssInclude("jquery-ui-timepicker-addon.css");

        ...
        while( $data = mysql_fetch_array($result) ) 
        {
            ...
            if( $attachment ) 
            {
                ...
                foreach( $attachment as $filename ) 
                {
                    $filename = substr($filename, 7);
                    $attachments[] = $filename;
                    $allattachments[] = array( "replyid" => $replyId, "i" => $attachmentCount, "filename" => $filename );
                    $attachmentCount++;
                }
            }

            $ascreplies[] = array( "id" => $replyId, "userid" => $userid, "contactid" => $contactid, "name" => ($admin ? $admin : $clientname), "email" => ($admin ? "" : $clientemail), "admin" => ($admin ? true : false), "user" => $user, "date" => $date, "message" => $message, "attachments" => $attachments, "rating" => $rating );
            $replies[] = $ascreplies;
        }

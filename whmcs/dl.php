                    ...
                    $productsWithMatchingDownload = WHMCS\Product\Product::whereHas("productDownloads", function($query) use ($downloadID)
{
    ...
    $query->where($download->getTable() . ".id", $downloadID);
        ...
        $fileurl = $downloads_dir . DIRECTORY_SEPARATOR . $filename;
...
if( $file_path_real === false || strpos($file_path_real, $folder_path_real . DIRECTORY_SEPARATOR) !== 0 ) 
{
    throw new WHMCS\Exception\Fatal(sprintf("File not found. Please contact support.", ""));
}
